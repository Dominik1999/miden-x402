//! x402 multi-payment + merchant batch settlement test.
//!
//! 5 sequential payments from one ADN note (same as test 6), then:
//! the merchant batch-consumes all 5 P2ID notes in one transaction.
//!
//! The facilitator returns P2ID note bytes with each consume response.
//! The merchant collects them, then settles all at once.
//!
//! Run:
//!   RUST_LOG=info cargo test --release --test adn_x402_merchant_settlement -- --ignored --nocapture
...
struct ConsumeResponse {
    ...
    tx_id: String,
    error: Option<String>,
    /// hex-encoded P2ID NoteFile::NoteWithProof bytes for the merchant
...
async fn facilitator_worker(
    ...
    let mut exported_p2id_ids: Vec<NoteId> = Vec::new();

    while let Some((req, resp_tx)) = rx.recv().await {
        let resp =
            process_consume(&mut client, facilitator_id, &req, &mut current_note, &mut used_note_ids, &mut exported_p2id_ids).await;
...
async fn process_consume(
    ...
    exported_p2id_ids: &mut Vec<NoteId>,
) -> ConsumeResponse {
    ...
    let fail = |msg: &str| ConsumeResponse {
        success: false,
        tx_id: msg.into(),
        error: Some(msg.into()),
    ...
    let note = if let Some(hex) = &req.note_file_hex {
        ...
        let (note, note_file_for_import) = match &note_file {
            ...
            _ => {
                return fail("expected NoteWithProof");
            }
        };
    ...
    for attempt in 0..60 {
        ...
        if attempt == 59 {
            return fail("not authenticated timeout");
        }
        ...
        .await
    {
        Ok(tx_id) => {
            ...
            // Also find the P2ID output note (2 storage items) for the merchant
            let mut p2id_hex = None;
            for record_candidate in client.get_output_notes(NoteFilter::All).await.unwrap() {
                let rid = record_candidate.id();
                if used_note_ids.contains(&rid)
                    || exported_p2id_ids.contains(&rid)
                    || current_note.as_ref().map(|n| n.id()) == Some(rid)
                {
                    continue;
                }
                if record_candidate.inclusion_proof().is_some() {
                    if let Ok(nf) = record_candidate.into_note_file(&NoteExportType::NoteWithProof) {
                        let nf_bytes = nf.to_bytes();
                        if let Ok(NoteFile::NoteWithProof(n, _)) = NoteFile::read_from_bytes(&nf_bytes) {
                            if n.recipient().storage().num_items() == 2 {
                                eprintln!("[facilitator] P2ID for merchant: {} ({} assets)", rid, n.assets().num_assets());
                                exported_p2id_ids.push(rid);
    ...
    if consume_resp.success {
        ...
        (
            ...
            Json(PaymentResponse {
                ...
                error: None,
                p2id_note_file_hex: consume_resp.p2id_note_file_hex,
            }),
        )
            .into_response()
    } else {
...
async fn adn_x402_merchant_settlement() -> anyhow::Result<()> {
    ...
    let mut collected_p2id_hexes: Vec<String> = Vec::new();
    ...
    for payment_num in 1..=NUM_PAYMENTS {
        ...
        if payment_resp.success {
            if let Some(p2id_hex) = payment_resp.p2id_note_file_hex {
                eprintln!("[agent] payment {payment_num} SUCCESS + P2ID note ({} bytes hex)", p2id_hex.len());
                collected_p2id_hexes.push(p2id_hex);
            } else {
                eprintln!("[agent] payment {payment_num} SUCCESS (no P2ID note returned)");
            }
        } else {
            eprintln!(
    ...
    eprintln!("[agent] collected {} P2ID notes for merchant settlement", collected_p2id_hexes.len());
    ...
    eprintln!("[merchant] settlement client ready: {}", merchant_id.to_hex());

    // Import all P2ID notes
    let mut p2id_notes: Vec<Note> = Vec::new();
    for (i, p2id_hex) in collected_p2id_hexes.iter().enumerate() {
        let p2id_bytes = hex::decode(p2id_hex)?;
        let nf = NoteFile::read_from_bytes(&p2id_bytes)
            .map_err(|e| anyhow::anyhow!("P2ID {i} decode: {e}"))?;
        let (note, nf_for_import) = match &nf {
            NoteFile::NoteWithProof(n, _) => (n.clone(), nf),
            _ => anyhow::bail!("P2ID {i}: expected NoteWithProof"),
        };
        eprintln!("[merchant] importing P2ID #{}: {} ({} assets)", i + 1, note.id(), note.assets().num_assets());
        merchant_client.import_notes(&[nf_for_import]).await?;
        p2id_notes.push(note);
    }

    // Wait for all notes to authenticate
    eprintln!("[merchant] waiting for all {} P2ID notes to authenticate...", p2id_notes.len());
    for attempt in 0..60 {
        merchant_client.sync_state().await?;
        let mut all_auth = true;
        for note in &p2id_notes {
            if let Ok(Some(record)) = merchant_client.get_input_note(note.id()).await {
                if !record.is_authenticated() {
                    all_auth = false;
                    break;
                }
        ...
        if all_auth {
            eprintln!("[merchant] all P2ID notes authenticated (attempt {attempt})");
            break;
        }
        if attempt == 59 {
            anyhow::bail!("timeout waiting for P2ID notes to authenticate");
        }
        tokio::time::sleep(Duration::from_secs(3)).await;
    }

    // Batch-consume all P2ID notes in one transaction
    eprintln!("[merchant] batch-consuming {} P2ID notes...", p2id_notes.len());
    let input_notes: Vec<(Note, Option<Word>)> = p2id_notes
        .into_iter()
        .map(|n| (n, None))
        .collect();
    let batch_req = TransactionRequestBuilder::new()
        ...
        .build()
        .map_err(|e| anyhow::anyhow!("batch consume build: {e:?}"))?;

    let settlement_tx = merchant_client
        .submit_new_transaction(merchant_id, batch_req)
        .await
        .map_err(|e| anyhow::anyhow!("batch consume: {e}"))?;

    let total_settled = DEBIT_PER_PAYMENT * NUM_PAYMENTS as u64;
    eprintln!("[merchant] ════════════════════════════════════════════════");
    eprintln!("[merchant] SETTLEMENT SUCCESS: tx={settlement_tx}");
    eprintln!("[merchant]   {NUM_PAYMENTS} P2ID notes consumed in ONE transaction");
    eprintln!("[merchant]   Total settled: {total_settled}");
    eprintln!("[merchant] ════════════════════════════════════════════════");

    eprintln!("\n══════════════════════════════════════════════════════");
    eprintln!("  FULL FLOW COMPLETE!");
    eprintln!("  ADN balance: {adn_balance}");
    eprintln!("  {NUM_PAYMENTS} payments of {DEBIT_PER_PAYMENT} each");
    eprintln!("  Merchant settled {total_settled} in 1 batch tx");
    eprintln!("  Remaining in ADN: {}", adn_balance - total_settled);
    eprintln!("══════════════════════════════════════════════════════");
